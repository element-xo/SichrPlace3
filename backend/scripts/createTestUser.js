 ...
 * Creates the test user with properly hashed password
 */
...
async function createTestUser() {
  ...
  try {
    ...
    // Also create sample apartments owned by this user
...
async function createSampleApartments(supabase, userId) {
  ...
  const apartments = [
    {
      ...
      rooms: 3,
      bathrooms: 1,
      available_from: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 1 week from now
      owner_id: userId,
    ...
    },
    {
      ...
      rooms: 1,
      bathrooms: 1,
      available_from: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 2 weeks from now
      owner_id: userId,
    ...
    },
    {
      ...
      rooms: 4,
      bathrooms: 2,
      available_from: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 1 month from now
      owner_id: userId,
  ...
  const { data, error } = await supabase
    .from('apartments')
    .upsert(apartments, { onConflict: 'title' })
    .select();
